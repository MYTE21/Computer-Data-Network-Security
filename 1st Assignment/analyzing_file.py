...
def analyzing_file():
    ...
    for word in words:
        if len(word) == 1:
            one_letter_freq = get_word_freq(one_letter_freq, word)
        if len(word) == 2:
            two_letter_freq = get_word_freq(two_letter_freq, word)
        if len(word) == 3:
            three_letter_freq = get_word_freq(three_letter_freq, word)
    ...
    print("#"*50, "Individual Character", "#"*50, "\n")
